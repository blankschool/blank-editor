...
const normalizeCode = [
  ...
  "  image_strategy: b.image_strategy === 'ai' ? 'ai' : 'stock',",
...
const pagesCode = [
  ...
  "  const visual = String(card.imagem_prompt || card.visual || card.imagem || (titulo + '. ' + corpo)).slice(0, 500);",
  "  const strategy = entrada.image_strategy;",
  "  return { layers: {",
  "  imagem: { asset: { strategy, ...(strategy === 'ai' ? { prompt: visual } : { query: visual }), aspectRatio: '4:5' } },",
  "  titulo: { text: titulo },",
  "  corpo: { text: corpo },",
  ...
  "  } };",
  "});",
  "const effectiveRunId = entrada.run_id || ('n8n-execution-' + $execution.id);",
  "return [{ json: {",
  ...
  "  idempotency_key: effectiveRunId,",
  "} }];",
].join("\n");
...
const prepareRowsCode = [
  ...
  "if (!design || !Array.isArray(design.pages) || !generation) throw new Error('Blank não devolveu design/generation completos');",
  `const reviewUrl = ${JSON.stringify(BLANK_ORIGIN)} + (design.reviewPath || design.editorPath);`,
  ...
  "  design_version: generation.version,",
...
const responseCode = [
  ...
  "  download_state: generated.review_status === 'approved' ? 'available' : 'blocked',",
...
revisedNodes.push(
  ...
  httpNode("Criar design no Blank", [1760, 300], {
    ...
    sendBody: true,
    specifyBody: "json",
    jsonBody: "={{ JSON.stringify({ template: $json.blank_template_id, name: $json.design_name, pages: $json.pages, run_id: $json.run_id }) }}",
...
try {
  ...
  const payload = { briefing_id: SMOKE_BRIEFING_ID, blank_template_id: TEMPLATE_ID, run_id: runId, image_strategy: "stock" };
