...
/**
 * Rasteriza a página INTEIRA como um PNG — não as camadas de imagem individuais (isso é
 * `extractPageImages`), a página completa como ela se vê. Usado só como entrada visual para o
 * matching de fonte por IA (server/src/render/googleFontMatch.ts): a página completa dá ao
 * modelo o contexto de layout/estilo que uma camada de texto isolada não tem.
 *
 * 150dpi é suficiente para o modelo de visão reconhecer o estilo de uma fonte sem gerar um PNG
 * gigante por página — este raster nunca vai para o design final, só para o prompt.
 */
