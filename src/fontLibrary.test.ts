...
import {
  ...
  catalogStylesheetUrls, designFamilies, fontLabel, fontStylesheetUrl, searchFontLibrary,
} from "./fontLibrary.ts";
...
test("the catalogue is fetched in batches, skipping what index.html already loads", () => {
  ...
  assert.deepEqual(
    [...requested].sort(),
    FONT_LIBRARY.map((f) => f.family).filter((f) => !PRELOADED_FAMILIES.includes(f)).sort(),
  );
